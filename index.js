...
<head>
    ...
    <style>
        ...
        .mini-card { width: 22%; height: 55px; background: #5c5042; color: #e0d0b8; border-radius: 5px; display: flex; flex-direction: column; align-items: center; justify-content: center; font-size: 11px; cursor: pointer; border: 1px solid #3d342b; }
        /* 미니카드 품절 시 색상 */
        .mini-card.sold-out { background: #1a1a1a; color: #555; border-color: #000; opacity: 0.6; }
...
<body>
    ...
    <div id="game-screen">
        <div id="top-bar">
            <div style="display: flex; justify-content: space-between;">
                <span id="room-info-display">대기실 (<span id="player-count">0</span>/4)</span>
                <button id="start-btn" onclick="requestStart()" style="display:none; background:#3e6b46; color:white; border:none; border-radius:5px; padding:5px 10px;">▶ 시작</button>
            </div>
            <div id="player-names-box" style="font-size:12px; margin-top:5px; text-align:left; color:#9c8e7e;"></div>
        </div>
        <div id="game-log"></div>
        <div id="turn-info" style="color:#e6b322; font-weight:bold; margin-bottom:5px;">입장 완료</div>
        ...
        <div id="mini-card-area">
            <div style="font-size:11px; color:#e6b322; margin-bottom:5px;">📋 남은 카드 (클릭 시 능력 확인)</div>
    ...
    <script>
        ...
        function joinGame() {
            ...
            if(!myName) return showNotice("이름을 입력하시오!");
        ...
        socket.on('updateHand', (hand) => {
            ...
            for(let i=0; i<2; i++) {
                ...
                } else { el.innerHTML = "패 없음"; el.classList.add('disabled'); }
            }
        });
        ...
        function clickCard(idx) {
            ...
            if((selectedCard.includes("자객") || selectedCard.includes("임금")) && myHand.some(c => c.includes("후궁"))) return showNotice("후궁을 먼저 버려야 하오!");
        ...
        function selectTarget(name) {
            ...
            if(selectedCard.includes("포졸") && selectedTarget !== myName) document.getElementById('guess-overlay').style.display = 'flex';
            else submitPlay();
        }
        ...
        socket.on('updateCardStats', (stats) => {
            document.getElementById('mini-card-container').innerHTML = stats.map(s => {
                ...
                <div class="mini-card ${isSoldOut}" onclick="showNotice('${s.emoji}${s.name}: ${cardInfo[s.num].desc}')">
                    <b>${s.emoji}${s.num}</b><span>${s.remaining}/${s.total}</span>
                </div>`;
            }).join("");
        });
    </script>
